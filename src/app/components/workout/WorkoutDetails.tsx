...
const WorkoutDetails = () => {
  ...
  if (loading) {
    return (
      <main className="flex min-h-[calc(100vh-88px)] items-center justify-center px-4">
        <p className="text-sm text-[#8F95A0]">Loading workout...</p>
      </main>
    );
  }
  ...
  const workout = workouts.find((item) => String(item.id) === cardid);
          ...
          <div className="flex flex-col">
            ...
            <div className="mt-7 flex flex-wrap gap-3">
              <button
                type="button"
                onClick={() =>
                  inPlan ? removeFromPlan(workout.id) : addToPlan(workout.id)
                }
              ...
              <button
                type="button"
                onClick={() =>
                  saved ? removeFromSaved(workout.id) : addToSaved(workout.id)
                }
...
const DetailRow = ({ label, value }: DetailRowProps) => {
  return (
    <div className="flex min-h-11.75 items-center justify-between border-b border-[#25282F] px-4 last:border-b-0">
      ...
      <span className="text-sm text-[#E1E3E7]">{value}</span>
    </div>
  );
};
